    ...
    try:
        ...
        if result["uses_snapshot_testing"]:
            completeness_factors = [
                ...
                result["snapshot_test_count"] >= 5,
                len(result["snapshot_extensions"]) > 0
            ]
            result["snapshot_completeness"] = sum(completeness_factors) / len(completeness_factors)

        # Calculate snapshot testing score (1-100 scale)
        score = 1  # Minimum score for a successful check
        if result["uses_snapshot_testing"]:
            score += 39  # Base points for using snapshot testing
            if result["framework_used"]:
                score += 10  # Points for using a known framework
            if result["has_inline_snapshots"] and result["has_file_snapshots"]:
                score += 10  # Diversity points
            elif result["has_inline_snapshots"] or result["has_file_snapshots"]:
                score += 5
            if result["has_snapshot_updates"]:
                score += 10  # Points for update configuration
            score += int(result["snapshot_completeness"] * 35)  # Granular completeness points

        result["snapshot_testing_score"] = min(max(round(score, 1), 1), 100)  # Ensure score is within 1-100 range
        return result

    except Exception as e:
        logger.error(f"Error during snapshot testing check: {e}")
        return {
            "uses_snapshot_testing": False,
            "snapshot_testing_score": 0,  # Score 0 for failed checks
            "error": str(e)
        }
