...
def timeout_handler(signum, frame):
    ...
    raise TimeoutException("Type safety check timed out")
...
def check_type_safety(repo_path: str = None, repo_data: Dict = None) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            for root, dirs, files in limited_walk(repo_path, max_depth=2):  # Limit to 2 levels deep
                ...
                if current_time - start_time > max_execution_time:
                    ...
                    result["performance_info"]["timeout_reason"] = "overall_execution"
                    break
        ...
        if len(eligible_files) <= 5:  # For tiny repos, avoid ThreadPoolExecutor overhead
            ...
            # Use a safety flag to force shutdown if needed
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                for future in as_completed(future_to_file):
                    ...
                    current_time = time.time()
                    if current_time - start_time > max_execution_time:
                        logger.warning(f"Overall execution timeout reached. Stopping analysis.")
                        result["performance_info"]["early_termination"] = True
                        result["performance_info"]["timeout_reason"] = "overall_execution"
                    ...
                    if current_time - analysis_start_time > analysis_timeout:
                        ...
                        result["performance_info"]["early_termination"] = True
                        result["performance_info"]["timeout_reason"] = "analysis_timeout"
                        executor.shutdown(wait=False)
                        break
                        
                    try:
                        # Short timeout for individual file result
                        result_data = future.result(timeout=1.0)  # Reduced from 1.5 to 1.0 seconds
        ...
        if time.time() - start_time < max_execution_time - 2:  # Only if we have at least 2 seconds left
        ...
        # Add note if execution was early terminated
        if execution_time >= max_execution_time or result["performance_info"]["early_termination"]:
            result["execution_note"] = f"Check was completed with {result['performance_info']['timeout_reason'] or 'timeout'}. Results may be partial."
        
        return result
    
    except TimeoutException:
        logger.error("Global timeout triggered for type safety check")
        # Return partial results if we got any
        return {
            "has_type_annotations": False,
            "has_type_checking": False,
            "type_safety_score": 25,  # Default score for timed out checks
            "typed_languages": [],
            "untyped_languages": [],
        ...
        logger.error(f"Unexpected error in type safety check: {str(e)}")
    ...
    try:
        ...
        def run_with_timeout():
            try:
                ...
                logger.error(f"Error in threaded type safety check: {e}", exc_info=True)
        ...
        if not result_container["done"]:
            logger.error("Type safety check thread did not complete in time")
            return {
                "status": "failed",
                "score": 0,
