...
def check_spdx_identifiers(repo_path: Optional[str] = None, repo_data: Optional[Dict] = None) -> Dict[str, Any]:
    ...
    if not repo_path or not os.path.isdir(repo_path):
        ...
        try:
            logger.debug(f" {repo_path} for SPDX identifiers")
...
def run_check(repository: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.debug(f"Starting SPDX identifiers check for repository: {repo_name}")
    ...
    try:
        ...
        logger.debug(f"✅ Completed SPDX identifiers check for {repo_name} with score: {final_result['score']}")
        return final_result
