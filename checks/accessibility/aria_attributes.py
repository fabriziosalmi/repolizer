...
def run_check(repository: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if cached_result:
        logger.debug(f"Using cached ARIA attributes check result for {repository.get('name', 'unknown')}")
    ...
    try:
        ...
        try:
            ...
            if start_time:
                ...
                logger.debug(f"✅ ARIA attributes check completed in {execution_time:.2f}s with score: {result.get('aria_usage_score', 0)}")
        except Exception as e:
            if start_time:
                execution_time = time.time() - start_time
