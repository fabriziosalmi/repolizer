...
def enqueue_output(pipe, process_id, is_error=False):
...
def start_scraper():
    ...
    try:
        ...
        stdout_thread = threading.Thread(
            ...
            args=(process.stdout, process_id, False),
            daemon=True
        )
        stderr_thread = threading.Thread(
            target=enqueue_output,
            args=(process.stderr, process_id, True),
            daemon=True
        )
...
def run_analyzer(job_id, config, job_queue):
    """Run the check_orchestrator to analyze a repository"""
    try:
        # Update job status
